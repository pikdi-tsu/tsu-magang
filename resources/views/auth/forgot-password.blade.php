...
<body class="bg-gradient-to-br from-[#4c9ca8] to-[#2c6e7a] font-sans min-h-screen flex items-center justify-center p-4">
    ...
    <div class="bg-white w-full max-w-[600px] rounded-[40px] shadow-2xl p-8 sm:p-12 flex flex-col items-center text-center">
        ...
        <form action="{{ route('password.email') }}" method="POST" class="w-full max-w-md text-left">
    @csrf

    <div class="mb-8">
        <label class="block text-black font-semibold text-sm mb-2 ml-1">
            Email
        </label>
        <input
            type="email"
            ...
            class="w-full border border-gray-400 px-4 py-3 rounded text-sm placeholder-gray-300 italic
